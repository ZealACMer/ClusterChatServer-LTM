...
int main(int argc, char **argv)
{
    ...
    // 初始化读写线程通信用的信号量
    sem_init(&rwsem, 0, 0); //第一个参数是信号量的地址，第二个参数代表线程间的通信，第三个参数代表信号量的初始值
    ...
    for (;;)
    {
        ...
        case 1: // login业务
        {
            ...
            g_isLoginSuccess = false; //每次登录之前，初始化为false
            ...
            sem_wait(&rwsem); // 等待信号量，由子线程处理完登录的响应消息后，通知这里，信号量-1
...
void doLoginResponse(json &responsejs)
{
    ...
    else // 登录成功
    {
        ...
        {
            // 初始化，防止loginout后，重新登录，g_currentUserGroupList中记录两份相同的数据信息
        ...
        if (responsejs.contains("offlinemsg"))
        {
            ...
            for (std::string &str : vec)
            {
                ...
                {
                    std::cout << "群消息[" << js["groupid"] << "]:" << js["time"].get<string>() << " [" << js["id"] << "]" << js["name"].get<string>()
                            << " said: " << js["msg"].get<string>() << std::endl;
                }
            }
        }
...
void readTaskHandler(int clientfd)
{
    for (;;)
    {
        ...
        if (LOGIN_MSG_ACK == msgtype)
        {
            ...
            sem_post(&rwsem);    // 通知主线程，登录结果处理完成，信号量+1
            continue;
        }
...
void groupchat(int clientfd, std::string str)
{
    ...
    int groupid = atoi(str.substr(0, idx).c_str()); //atoi处理的是char*
    std::string message = str.substr(idx + 1, str.size() - idx);
